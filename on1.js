(function(){
  ...
  // --- Функции авторизации (оставлены без изменений) ---
  function getToken(){
    try{
  ...
  // Отключаем серверную подмену бандла, чтобы наши изменения сохранились
  ...
  function checkToken(tok){
    ...
    statusReq('token='+encodeURIComponent(tok)+(uid?'&uid='+encodeURIComponent(uid):'')+(cub?'&cub='+encodeURIComponent(cub):''),function(r){
      ...
      try{
        var ls=localStorage.getItem(LS_TOK);
        if(ls&&ls!==tok){saveToken(ls);checkToken(ls);return;}
      }catch(e){}
      tryRecovery();
    },function(){});
  }
...
(function () {
  ...
  // ЗАГРУЖАЕМ СРАЗУ ДВА ПЛАГИНА (Alpac + Cinema)
  var LIST = [
    {"k":"online","o":0,"u":"https://beta.l-vid.online/online.js"},      // Основа: Alpac v0.5
    {"k":"cinema","o":0,"u":"https://bylampa.github.io/cinema.js"},      // Дополнение: Источник Alloha
  ...
  function loadBundle() {
    var timer = setInterval(function () {
      if (typeof Lampa !== 'undefined') {
        ...
        // --- ДОБАВЛЕНИЕ СМАЙЛИКОВ НАЧАЛО ---
        try {
          var addEmoji = function(title) {
            if (typeof title !== 'string') return title;
            // Если текст совпадает с alloha (без учета регистра), добавляем ананас
            if (/^alloha$/i.test(title)) return '🍍 Alloha';
            // Если filmix или filmixtv, добавляем хлопушку
            if (/^filmix(?:tv)?$/i.test(title)) return '🎬 Filmix';
            return title;
          };

          // Перехватываем верхнее меню выбора (вкладки)
          if (Lampa.Filter) {
          ...
          // Перехватываем выпадающие списки (например, при нажатии "Изменить балансер")
          if (Lampa.Select) {
